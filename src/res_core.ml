...
let parseStringLiteral s =
  ...
  let rec parse state i d =
    if i = len then
      match state with
      | HexEscape | DecimalEscape | OctalEscape | UnicodeEscape
      | UnicodeCodePointEscape ->
        false
      | _ -> true
    else
      ...
      | DecimalEscape ->
        if d == 2 then
          ...
          if c < 0 || c > 255 then false
            ...
            Buffer.add_char b (Char.unsafe_chr c);
            parse Start (i + 1) 0)
        else parse DecimalEscape (i + 1) (d + 1)
      | OctalEscape ->
        if d == 2 then
          let c0 = String.unsafe_get s (i - 2) in
          let c1 = String.unsafe_get s (i - 1) in
          ...
          if c < 0 || c > 255 then false
            ...
            Buffer.add_char b (Char.unsafe_chr c);
            parse Start (i + 1) 0)
        else parse OctalEscape (i + 1) (d + 1)
      | UnicodeEscapeStart -> (
        match c with
      ...
      | UnicodeEscape ->
        ...
        else parse UnicodeEscape (i + 1) (d + 1)
      | UnicodeCodePointEscape -> (
        match c with
        | '0' .. '9' | 'a' .. 'f' | 'A' .. 'F' ->
...
let parseHashIdent ~startPos p =
  ...
  | String text ->
    let text =
      if p.mode = ParseForTypeChecker then parseStringLiteral text else text
    in
    Parser.next p;
